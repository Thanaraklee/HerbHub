...
class LoadHerbtoNeo4j:
    def __init__(self, uri: str, auth: tuple, json_data: dict = None):
    ...
    @staticmethod
    def clear_database(uri, auth, database="graphherb"):
        try:
            GraphDatabase.driver(uri, auth=auth, database=database).session().run("MATCH (n) DETACH DELETE n")
    ...
    def ingest_json_species_name(self):
        ...
        query = """
        MERGE (:Species {name: $name, name_ref: $name_ref, link_ref: $link_ref})
        """
        try:
            self.db.run(query, name=name, name_ref=name_ref, link_ref=link_ref)
